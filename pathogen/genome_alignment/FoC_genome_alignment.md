...
```bash
  Reference=$(ls repeat_masked/F.oxysporum_fsp_cepae/Fus2/filtered_contigs_repmask/Fus2_contigs_softmasked.fa)
...
```bash
  ...
  for StrainPath in $(ls -d qc_dna/paired/F.*/HB6); do
    ...
    Organism=$(echo $StrainPath | rev | cut -f2 -d '/' | rev)
    echo "$Organism - $Strain"
    F1_Read=$(ls $StrainPath/F/HB6_S4_L001_R1_001_trim.fq.gz);
    R1_Read=$(ls $StrainPath/R/HB6_S4_L001_R2_001_trim.fq.gz);
    F2_Read=$(ls $StrainPath/F/HB6_S5_L001_R1_001_trim.fq.gz);
    R2_Read=$(ls $StrainPath/R/HB6_S5_L001_R2_001_trim.fq.gz);
    echo $F1_Read
    echo $R1_Read
    echo $F2_Read
    echo $R2_Read
