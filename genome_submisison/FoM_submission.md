...
printf "FOMA001 SAMN07508183 Stocks4" \
> genome_submission/FoMatthioli_PRJNA506531_locus_tags.txt
```
...
```bash
for Assembly in $(ls repeat_masked/*/*/filtered_contigs/*_contigs_softmasked_repeatmasker_TPSI_appended.fa | grep -e 'Stocks4'); do
...
cat genome_submission/FoL_submission.sbt | sed -E 's/Lactucae/Matthioli/g' | sed -E "s/SAMN\w*/SAMN07508183/g" | sed -E "s/SUB\w*/SAMN07508183/g" | sed -E "s/PRJNA\w*/PRJNA398321/g" > genome_submission/FoM_submission.sbt
SbtFile="genome_submission/FoM_submission.sbt"
LabID="ArmitageEMR"
```
...
for Assembly in $(ls repeat_masked/*/*/filtered_contigs/*_contigs_softmasked_repeatmasker_TPSI_appended.fa  | grep -e 'Stocks4'); do
    ...
    echo "$Organism - $Strain"
    OutDir="genome_submission/$Organism/$Strain"
    LocusTag=$(cat genome_submission/FoMatthioli_PRJNA506531_locus_tags.txt | grep "$Strain" | cut -f1 -d ' ')
    echo $LocusTag
...
for Assembly in $(ls repeat_masked/*/*/filtered_contigs/*_contigs_softmasked_repeatmasker_TPSI_appended.fa | grep -e 'Stocks4'); do
  Strain=$(echo $Assembly| rev | cut -d '/' -f3 | rev)
  Organism=$(echo $Assembly | rev | cut -d '/' -f4 | rev)
  Taxon=$(echo $Organism | sed 's/_/ /g' | sed 's/F./Fusarium /g' | sed 's/fsp/f. sp./g' | sed 's/mathioli/mattholi/g')
  echo "$Taxon - $Strain"
  OutDir="genome_submission/$Organism/$Strain"
