...
```bash
  # For FoC 55
  TarBall=$(ls /data/seq_data/pacbio/2017/John_Clarkson_UWAR.JC.ENQ-1929/John_Clarkson_UWAR.JC.ENQ-1929_R_S4.tar.gz)
  ScratchDir=/home/scratch/groups/harrisonlab/fusarium/F.oxysporum_f.sp.cepae/55
  mkdir -p $ScratchDir
  tar -C $ScratchDir -zxvf $TarBall
  cat $ScratchDir/*/*/*/Analysis_Results/*.subreads.fastq | gzip -cf > $ScratchDir/concatenated_pacbio.fastq.gz
  OutDir=raw_dna/pacbio/F.oxysporum_fsp_cepae/55/extracted
  mkdir -p $OutDir
  cp -s $ScratchDir/concatenated_pacbio.fastq.gz $OutDir/.
  # For Fo fo47
  TarBall=$(ls /data/seq_data/pacbio/2017/John_Clarkson_UWAR.JC.ENQ-1929/John_Clarkson_UWAR.JC.ENQ-1929_S5.tar.gz)
  ScratchDir=/home/scratch/groups/harrisonlab/fusarium/F.oxysporum/fo47
  mkdir -p $ScratchDir
  tar -C $ScratchDir -zxvf $TarBall
  cat $ScratchDir/data/filtered_subreads.fastq | gzip -cf > $ScratchDir/concatenated_pacbio.fastq.gz
  OutDir=raw_dna/pacbio/F.oxysporum/fo47/extracted
  mkdir -p $OutDir
  cp -s $ScratchDir/concatenated_pacbio.fastq.gz $OutDir/.
```
...
```bash
  for Reads in $(ls raw_dna/pacbio/F.*/*/extracted/concatenated_pacbio.fastq.gz | grep -e 'fo47' -e '55'); do
    GenomeSz="53m"
    Strain=$(echo $Reads | rev | cut -f3 -d '/' | rev)
    Organism=$(echo $Reads | rev | cut -f4 -d '/' | rev)
    Prefix="$Strain"_canu
    OutDir="assembly/canu-1.4/$Organism/$Strain"_canu
...
for PacBioDat in $(ls raw_dna/pacbio/F.*/*/extracted/concatenated_pacbio.fastq.gz | grep -e 'fo47' -e '55' | grep '55'); do
echo $StrainPath
...
# qsub $ProgDir/sub_spades_pacbio.sh $PacBioDat $TrimF1_Read $TrimR1_Read $OutDir
OutDir=assembly/spades_pacbio/$Organism/"$Strain"
Cutoff='10'
qsub $ProgDir/sub_spades_pacbio.sh $PacBioDat $TrimF1_Read $TrimR1_Read $OutDir $Cutoff
done
```
